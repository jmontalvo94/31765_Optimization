...
for weight=0:1:10;
    ...
    Objective = sum(alfa_k,1)+weight*trace(Y_k_(1)*W)+weight*trace(Y_k_(2)*W)+weight*trace(Y_k_(3)*W);
    ...
    objectiveSDP=[objectiveSDP, value(sum(alfa_k,1))+weight*trace(Y_k_(1)*W)+weight*trace(Y_k_(2)*W)+weight*trace(Y_k_(3)*W)]
    objectiveACOPF = [objectiveACOPF, nonconvex_results.f]
    success = [success, nonconvex_results.success]
    ...
    %     ylabel('Eigenvalue')
        ...
        ratioSDP_slack=[ratioSDP_slack,ratio]
    ...
    %     xlabel('Eigenvalue Index')
    %     ylabel('Eigenvalue')
        ratio=abs(lambda(2))/abs(lambda(3));
        ratioSDP=[ratioSDP,ratio];
...
if slack==1
    semilogy(0:1:10,ratioSDP_slack)
    ylabel('Eigenvalue Ratio')
    xlabel('Penalty weight')
    set(gca,'XDir')
else
    semilogy(0:1:10,ratioSDP)
    ylabel('Eigenvalue Ratio')
    xlabel('Penalty weight')
    set(gca,'XDir')
end